...
def find_matching_image(image_folder, txt_filename):
    basename = os.path.splitext(txt_filename)[0]
    extensions = [".png", ".jpg", ".jpeg", ".JPG"]
    for ext in extensions:
        image_path = os.path.join(image_folder, basename + ext)
        if os.path.exists(image_path):
            return image_path
    return None  # ไม่พบภาพ
...
def main():
    ...
    for filename in os.listdir(INPUT_FOLDER):
        if filename.lower().endswith('.txt'):
            ...
            image_path = find_matching_image("slip_images", filename)
            ...
            if bank == 'scb':
                data = handle_scb(text, image_path)
            elif bank == 'krungthai':
                data = handle_krungthai(text, image_path)
            elif bank == 'kbank':
                data = handle_kbank(text, image_path)
            elif bank == 'bangkok':
                data = handle_bangkok(text, image_path)
            elif bank == 'gsb':
                data = handle_gsb(text, image_path)
            else:
                handle_unknown(text, image_path)
                data = {}
